...
public class Harvest {
    public static void main(String[] args) {
        ...
        double grapemeters = Double.parseDouble(scanner.nextLine());
        int sellwine = Integer.parseInt(scanner.nextLine());
        int workers = Integer.parseInt(scanner.nextLine());

        double havegrape = meters * grapemeters;
        double winemeters = havegrape - (havegrape * 60 / 100);
        double wineliters = winemeters / 2.5;


        if (sellwine > wineliters) {
            double total1 = Math.floor(sellwine - wineliters);
            System.out.printf("It will be a tough winter! More %.0f liters wine needed.", total1);
        } else if (wineliters >= sellwine) {

            double wineliters1 = Math.floor(wineliters);
            double total2 = Math.ceil(wineliters - sellwine);
            double total3 = Math.ceil(total2 / workers);

            System.out.printf("Good harvest this year! Total wine: %.0f liters.", wineliters1);
            System.out.println();
            System.out.printf("%.0f liters left -> %.0f liters per person.",total2, total3);
        }
